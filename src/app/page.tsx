...
export default function Home() {
  ...
  const startRecording = async () => {
    ...
    if (!(navigator.mediaDevices && navigator.mediaDevices.getUserMedia)) {
        setAudioError("Audio recording not supported.");
        return;
    }
    try {
        ...
        <div className="flex-1 flex flex-col overflow-y-hidden p-4 gap-2">
          <div className="flex justify-between items-center flex-shrink-0 gap-4 border-b pb-2 h-12">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              {/* --- Recording State: Timer, Buttons --- */} 
              {isRecording && !isProcessingAudio && (
                <div className="flex items-center justify-start gap-2 w-full min-w-[250px]"> 

                  {/* Timer Display (First) - Ensure no wrap */} 
                  <span 
                    className={clsx(
                      "text-sm font-mono flex-shrink-0 whitespace-nowrap", // Removed fixed width/text-left, added no-wrap
